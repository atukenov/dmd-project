...
paymentSchema.index({ qrCodeExpiry: 1 }, { sparse: true });
...
// Static methods
// eslint-disable-next-line @typescript-eslint/no-explicit-any
paymentSchema.statics.findByBusiness = function(businessId: string, filters: Record<string, any> = {}) {
  return this.find({ businessId, ...filters });
};

paymentSchema.statics.findActivePayments = function(businessId?: string) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const query: Record<string, any> = { status: { $in: ['pending', 'processing'] } };
  if (businessId) {
...
paymentSchema.statics.getPaymentStats = function(businessId: string, dateFrom?: Date, dateTo?: Date) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
